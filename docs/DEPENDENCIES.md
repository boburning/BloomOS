...
| rcheevos hashing | Official v12.4.0 commit `2ad0b8672f68a48148620164510b963039e49eb1` (annotated tag object `7572a3b86e7dac4b2f053f4a21b7793af093e97b`) plus zlib 1.3.1 commit `51b7f2abdade71cd9bb0e7a373ef2610ec6f9daf` | Sources and MIT/zlib licenses pinned | Bloom compiles upstream `rc_hash` and the bounded minizip reader into `bloom-ra`; cartridge ZIPs are extracted in memory and then passed unchanged to rcheevos. Arcade ZIP hashing remains under rcheevos control. |
| Bloom disc hash bridge | rcheevos `2ad0b867...`, libchdr `6cde5348...`, and RAOfflineProxy glue `be6898e6...`, all byte/SHA locked in `build/raofflineproxy/sources.json` | Source-built GPL-3.0-only bridge with MIT/BSD dependencies | `libbloom-rchash.so` supplies authoritative CHD and PBP reader callbacks to `bloom-ra`. It ships in the core runtime and does not require the optional RAOfflineProxy service. Bloom validates the requested file against its ROM root before invoking the bridge; playlist references remain on the separately guarded path. |
...
| OpenSSH SFTP subsystem | Official OpenSSH portable `9.9p2`, commit `d76b2675116617394cd355a3437b4963a562b64d` | Tag and commit pinned | Bloom builds only the ARMv7 `sftp-server` subsystem without OpenSSL, PAM, or zlib. A Bloom gate exposes it only when SFTP is explicitly enabled through the key-only SSH service. `LICENSE.openssh` ships beside it. |
