...
| 2026-08-16 | Miyoo Mini Plus (model ID 354) | `v0.1.0-dev-a93f9426` | Upgrade install, boot, identity, display/input inventory, Wi-Fi association, shutdown | Clean install and boot; safe-only request completed; Wi-Fi connected; normal shutdown completed | Physical panel, early battery probe, and Wi-Fi capability reporting were corrected from the captured evidence. Windows found no FAT damage but observed the recurring dirty bit; the next build explicitly disables SD swap and remounts FAT read-only before poweroff. |
